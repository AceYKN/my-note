...
| :--- | :---: | :---: | :---: | :---: | :--- |
| **整数 $\mathbb{Z}$** | 是 | 是 | 是 | 否 (仅 $\pm 1$) | **整环** (Integral Domain) |
| **有理数 $\mathbb{Q}$** | 是 | 是 | 是 | 是 | **域** (Field) |
| **矩阵 $M_n(\mathbb{R})$** | 否 | 是 | 否 | 否 (仅非奇异矩阵) | 非交换幺环 (有零因子) |
| **模 $n$ 整数 $\mathbb{Z}_n$** | 是 | 是 | 取决于 $n$ | 取决于 $n$ | 若 $n$ 为合数，则有零因子；<br>若 $n$ 为素数，则是**域** |
| **多项式 $R[x]$** | 若 $R$ 交换 | 是 | 若 $R$ 无零因子 | 否 (仅常数项可逆) | 若 $R$ 是整环，则 $R[x]$ 是整环 |
