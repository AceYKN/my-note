...
- 逻辑地址
  程序中使用的地址，又称虚拟地址，是进程看到的地址，需要经过地址转换才能变成物理地址。

外部碎片
内存中存在许多不连续的小空闲区，总空闲空间可能足够，但因为不连续，无法分配给需要连续空间的进程。

i-node
UNIX/Linux 文件系统中的索引节点，用来保存文件的元数据和数据块地址信息，如文件类型、权限、大小、时间、链接计数、直接/间接地址项等。

活锁
进程没有被阻塞，也在不断改变状态，但由于彼此反复让步或重试，始终无法向前推进完成任务。

临界区
进程中访问共享资源或共享数据的那段代码。为了避免竞争条件，同一时刻通常只允许一个进程进入相关临界区。

中断
由硬件或软件事件引起的处理器控制转移机制，使 CPU 暂停当前执行，转去执行相应的中断处理程序。

字符设备
以字符流为单位进行输入输出的设备，通常不支持随机访问，如键盘、鼠标、串口、打印机等。

用户接口
操作系统提供给用户与计算机系统交互的方式，包括命令行接口、图形用户接口和系统调用接口等。

并行
多个任务在同一时刻真正同时执行，通常需要多个处理器或多个核心支持。

饥饿
某个进程长期得不到所需资源或 CPU 服务，虽然系统整体在运行，但该进程一直无法继续执行。

文件管理系统
操作系统中负责文件存储、组织、访问、保护和目录管理的部分，为用户提供按名存取文件的功能。

FAT
File Allocation Table，文件分配表。它记录磁盘中各文件数据块之间的链接关系，用于管理文件的磁盘空间分配。

索引文件
通过索引表来组织和访问记录的文件。索引项通常保存关键字和对应记录地址，可以提高查找速度。

相对地址
相对于某个基准地址或程序起始位置的地址，也称偏移地址。实际访问时需与基址相加形成物理地址或有效地址。
